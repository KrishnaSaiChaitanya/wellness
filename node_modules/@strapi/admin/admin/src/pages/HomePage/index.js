...
export const HomePageCE = () => {
  ...
  useEffect(() => {
    const apiUrl = 'https://welleness.onrender.com/api/results';
    fetch(apiUrl)
     ...
     const fetchData = async () => {
      try {
        const ownersResponse = await fetch('https://welleness.onrender.com/api/owners');
        const ownersData = await ownersResponse.json();
        console.log(ownersData, "this is data");
        setOwnersCount(ownersData.data.length);

        const breedersResponse = await fetch('https://welleness.onrender.com/api/breeders');
        const breedersData = await breedersResponse.json();
        setBreedersCount(breedersData.data.length);

        const veterinariansResponse = await fetch('https://welleness.onrender.com/api/veterinarians');
        const veterinariansData = await veterinariansResponse.json();
        setVeterinariansCount(veterinariansData.data.length);

        const testKitsResponse = await fetch('https://welleness.onrender.com/api/test-kits');
        const testKitsData = await testKitsResponse.json();
        setTestKitsCount(testKitsData.data.length);

        const testsResponse = await fetch('https://welleness.onrender.com/api/horse-infos');
        const testsData = await testsResponse.json();
        setTestsCount(testsData.data.length);

        const trainersResponse = await fetch('https://welleness.onrender.com/api/trainers'); // New API endpoint
      ...
      <GridGap style = {{padding: "20px"}}>
        {data.slice(0, 6).map((item , id) => {
          return (
            ...
            <img src={"https://res.cloudinary.com/dsfems7vy/image/upload/v1691442511/large_horse_9d7cf695b1.jpg"} style={{height: "44px", borderRadius: "30px"}}/>
